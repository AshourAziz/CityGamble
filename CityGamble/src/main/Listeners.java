	...
	public void onInventoryClick(InventoryClickEvent event) {
		...
		new BukkitRunnable() {
			public void run() {
				...
				if (API.getBroadcastCount() > 3 && API.ticketsSold() > 2) {
					String winner = API.draw();
					API.setCachedWinnner(winner);
					API.winnerPayOut(API.whatIsCachedWinner());
					API.newRound();
				}

			}
		}.runTaskTimer(Main.getInstance(), 1200l, 600L);
